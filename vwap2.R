...
# Function to calculate trend (using a simple moving average)
...
# VWAP-based trading logic with adjustments
trade_stock_vwap <- function(ticker, position, bid_ask, vwap, volatility) {
  ...
  # Dynamic VWAP threshold based on volatility
  vwap_threshold <- ifelse(volatility == "High", 0.005, 0.003)
  
  # Adjust logic based on ticker-specific performance
  if (ticker == "DUCK" || ticker == "OWL") {
    ...
    # Conservative trading for CROW and DOVE
    if (!is.null(best_bid_price) && (vwap - best_bid_price) / vwap > vwap_threshold && position < MAX_LONG_EXPOSURE) {
      POST(paste0(BASE_URL, "/orders"), 
...
main <- function() {
  ...
  repeat {
    ...
    for (ticker in ticker_list) {
      ...
      # Adjust strategies for specific tickers
      if (ticker == "DOVE" || ticker == "CROW") {
        # Reduce trading frequency for tickers with negative rebates
        if (!is.null(vwap) && abs(position) < 2 * ORDER_LIMIT) {
          trade_stock_vwap(ticker, position, bid_ask, vwap, trend)
        }
      } else {
        # Standard trading for OWL and DUCK
        trade_stock_vwap(ticker, position, bid_ask, vwap, trend)
      }
